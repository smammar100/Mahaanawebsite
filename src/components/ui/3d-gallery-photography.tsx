...
function GalleryScene({
  ...
  const spatialPositions = useMemo(() => {
    ...
    for (let i = 0; i < visibleCount; i++) {
      // Distribute evenly: left side (negative x) and right side (positive x)
      const isLeft = i < half;
      const sideIndex = isLeft ? i : i - half;
      const sideCount = isLeft ? half : visibleCount - half;
      const angle = (sideIndex / Math.max(sideCount, 1)) * Math.PI * 0.9;
      const radius = 0.4 + (sideIndex % 3) * 0.4;
      const x =
        (isLeft ? -1 : 1) *
        radius *
        maxHorizontalOffset *
        (0.6 + 0.4 * Math.sin(angle));
